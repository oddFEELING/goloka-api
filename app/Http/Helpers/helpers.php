...
function reCaptcha()
{
    $reCaptcha = \App\Models\Extension::where('act', 'google-recaptcha2')->where('status', 1)->first();
...
function analytics()
{
    $analytics = \App\Models\Extension::where('act', 'google-analytics')->where('status', 1)->first();
    return $analytics ? $analytics->generateScript() : '';
}

function tawkto()
{
    $tawkto = \App\Models\Extension::where('act', 'tawk-chat')->where('status', 1)->first();
...
function sidenavActive($routeName, $type = null)
{
    if ($type == 1) {
        $class = 'active open';
    } elseif ($type == 2) {
...
function imagePath()
{
    ...
    $data['profile'] = [
        ...
        'surveyor' => [
            'path' => 'assets/surveyor/images/profile',
            'size' => '350x300',
        ],
    ];
    return $data;
