...
class SurveyorTicketController extends Controller
{
    public function supportTicket()
    {
        if (auth()->guard('surveyor')->user()->id == null) {
            abort(404);
        }
        $page_title = "Support Tickets";
        $supports = SupportTicket::where('surveyor_id', auth()->guard('surveyor')->user()->id)->latest()->paginate(15);
        $empty_message = 'No support ticket found';
        return view('surveyor.support.index', compact('supports', 'page_title','empty_message'));
    }


    public function openSupportTicket()
    {
        if (!auth()->guard('surveyor')->user()->id) {
            abort(404);
        }
        $page_title = "Support Tickets";
        $surveyor = auth()->guard('surveyor')->user();
        return view('surveyor.support.create', compact('page_title', 'surveyor'));
    }
    ...
    public function storeSupportTicket(Request $request)
    {
        ...
        $ticket->surveyor_id = auth()->guard('surveyor')->user()->id;
        ...
        return redirect()->route('surveyor.ticket')->withNotify($notify);
    }
    ...
    public function viewTicket($ticket)
    {
        ...
        $surveyor = auth()->guard('surveyor')->user();
        return view('surveyor.support.view', compact('my_ticket', 'messages', 'page_title', 'surveyor'));
