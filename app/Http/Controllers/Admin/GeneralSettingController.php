...
class GeneralSettingController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        $request->merge(['surveyor_registration' => isset($request->surveyor_registration) ? 1 : 0]);
        $request->merge(['survey_approval' => isset($request->survey_approval) ? 1 : 0]);

        $general_setting->update($request->only(['sitename', 'cur_text', 'cur_sym', 'ev', 'en', 'sv', 'sn', 'registration',
            'surveyor_registration', 'survey_approval', 'base_color', 'secondary_color', 'get_amount', 'paid_amount']));
