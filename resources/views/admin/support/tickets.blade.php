                            ...
                            @forelse($items as $item)
                                <tr>
                                    ...
                                    <td data-label="@lang('Submitted By')">
                                        @if($item->user_id)
                                            <span class="badge badge--success">@lang('User')</span> <a href="{{ route('admin.users.detail', $item->user_id)}}"> {{@$item->user->fullname}}</a>
                                        @elseif($item->surveyor_id)
                                            <span class="badge badge--success">@lang('Surveyor')</span> <a href="{{ route('admin.surveyors.detail', $item->surveyor_id)}}"> {{@$item->surveyor->fullname}}</a>
                                        @else
                                            <span class="badge badge--success">@lang('Site')</span> <a href="#0"> {{@$item->name}}</a>
                                        @endif
                                    </td>
